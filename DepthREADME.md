...
- Spring Data JPA만 사용해서 20개의 API를 개발해 보았는데, save()와 findBy~() 함수등이 실제 어떻게 구현되어 있는지, 연관관계 매핑 방식을 어떻게 적용하는게 최적일지와 가 구현한 코드가 너무 복잡한건 아닌가 라는 궁금증이 생겼습니다. 그래서 다음주에는 추가적인 API를 구현하지 않고 유지보수 위주로 일정을 진행하겠습니다. 남은 시간은 Java와 Spring 기초, 알고리즘 공부로 채우도록 하겠습니다. 
